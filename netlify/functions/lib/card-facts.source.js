// card-facts.source.js — SINGLE SOURCE for inventory card pills. Edit this file only,
// then run: node scripts/generate-card-facts.js  (verify: node scripts/verify-card-facts.js)
// The generator wraps this body into three mirrors that must stay byte-identical in logic:
//   netlify/functions/lib/card-facts.generated.js  (CJS, tests)
//   netlify/edge-functions/lib/card-facts.esm.js    (edge SSR: /inventory first cards, category hubs)
//   js/card-facts.browser.js                        (client: window.CardFacts)
// Free identifiers provided by each wrapper: showMileage(unit), showHours(unit).
// The wrapper also decides the gate's failure mode: the edge/CJS mirrors import the real
// showMileage/showHours (fail-closed, as the edge has always behaved); the browser mirror
// wraps window.UsageDisplay and returns true when it is absent (fail-open, as the live client
// has always behaved — js/inventory-engine.js:425-427 at cc687342). Preserving both is what
// keeps this a refactor rather than a behaviour change.
//
// Two parts, deliberately separate:
//   A. LIVE  — buildCardChips(u, opts): today's informational pills. NEW is a status badge
//      OUTSIDE the two-informational-pill cap (Chief ruling M3, 2026-09-22). Stock # is
//      rendered by the callers, outside this function.
//      Chief ruling L1 (2026-09-22): the seven emitters that no caller can fetch —
//      transmission, drivetrain, operating_weight, length, gvwr, axles, deck_width — are
//      DELETED from the emitter map and from every order profile. None can fire: the
//      inventory_cards view exposes no such column (transmission and drivetrain exist on the
//      base inventory table but are not projected). Resulting profiles: mowers
//      ['hours','horsepower'], trailers [], trucks ['mileage','engine','fuel'], construction
//      ['horsepower','hours']. The four profiles containing no dead emitter — tractor, crane
//      truck, farm, default — are unchanged.
//      Chief ruling L2: NEW stays INSIDE the search_pills 3-slot limit.
//      Chief ruling L3: no switch restores superseded behaviour; opts carries only searchPills.
//      GOVERNED COHORT PILOT (Chief, 2026-09-22): for the adjudicated cohort below, and ONLY
//      for it, the informational pills come from Section B (governedCandidates ->
//      selectBestFacts) instead of the legacy emitters. Outside the cohort the legacy path is
//      byte-identical to cc687342/eb268691. opts also carries `title`, supplied by the caller
//      from the display title it has already computed, so Section B's duplicate suppression
//      can see the visible identity. No new title logic exists here.
//   B. DARK  — governed best-fact selection (governedCandidates → selectBestFacts → formatFact).
//      Not called by any renderer. Activation is a separate Chief gate.
//      Qualification (fact key + source + trust) is owned by the inventory_cards.governed_facts
//      view expression; this code never sees source or trust.
